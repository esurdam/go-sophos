...
// Get retrieves node {{.Path}}'s value from the gateway utilizing a sophos.ClientInterface
func({{firstLetter .Name}} *{{.Name}})Get(client sophos.ClientInterface)(err error) {
	{{firstLetter .Name}}.Value, err = Get{{.Name}}(client)
	return err
}

// Update PUT's' {{.Path}}'s value to the gateway utilizing a sophos.ClientInterface
...
// Get{{.Name}} gets the {{.Path}} node value
...
// Update{{.Name}} updates the {{.Path}} node value
...
var nodesHeader = `// Package nodes contains generated types and Get/Update functions for sophos.Node(s)
//
// This file was generated by bin/gen.go! DO NOT EDIT!
package nodes
...
func handleNodesNode() error {
	...
	f2, err := os.Create("nodes/nodes.go")
	if err != nil {
		log.Fatal(err)
	}
	f2.Write([]byte(nodesHeader))
	//
	...
	f, err := os.Create("nodes/generated.go")
	if err != nil {
		log.Fatal(err)
	}
	...
	f3, err := os.Create("nodes/directory.go")
	if err != nil {
		log.Fatal(err)
	}
